###############################################################################################################
###### Simulations to compute the Empirical power for PATH graph for IUT with K=3 experimental groups  ########
###############################################################################################################


rm(list = ls())
true.mean <- c(3.9908, 3.8108, 3.6306)                              # Sample mean of example
K<-length(true.mean)                                                # No. of experimental groups
psd<-0.1870                                                         # Poolad standard deviation
 
n_m<- c(28,56,28)  # max-min design
n_b= c(38,37,37)   # balance design
...
for (i in 1:m) {  #simulating from max-min design
  for (j in 1:K){
  X.m <- rnorm(n_m[j],true.mean[2],psd)  # null data from max--min design
  X.m_mean[j] <- mean(X.m)
  
  X.b <- rnorm(n_b[j],true.mean[2],psd)  # null data from balanced design
  X.b_mean[j] <- mean(X.b)
  }
  Z_12_m = (X.m_mean[1]-X.m_mean[2])/(psd*sqrt((1/n_m[1])+(1/n_m[2])))
  Z_23_m = (X.m_mean[2]-X.m_mean[3])/(psd*sqrt((1/n_m[2])+(1/n_m[3])))
  t_stat_m[i]<- min(c(abs(Z_12_m),abs(Z_23_m)))                          #test statistics under the null for max-min design

  Z_12_b = (X.b_mean[1]-X.b_mean[2])/(psd*sqrt((1/n_b[1])+(1/n_b[2])))
  Z_23_b = (X.b_mean[2]-X.b_mean[3])/(psd*sqrt((1/n_b[2])+(1/n_b[3])))
  t_stat_b[i]<- min(c(abs(Z_12_b),abs(Z_23_b)))                          #test statistics under the null for balanced design
...
for (j in 1:K){
  ...
  t_stat_m.A<- min(c(abs(Z_12_m.A),abs(Z_23_m.A)))   #test statistics under the altenative for max-min design
  ...
  t_stat_b.A<- min(c(abs(Z_12_b.A),abs(Z_23_b.A)))                          #test statistics under the altenative for balanced design
